...
var openApi2ConversionHelpers = require('./openApi2ConversionHelpers');
var openApi3ConversionHelpers = require("./helpers/v3/openApi3ConversionHelpers");
var resourceHelpersV3 = require('./helpers/v3/resourceHelpers');
var sortTags = resourceHelpersV3.sortTags;
var appendToApi = resourceHelpersV3.appendToApi;
var params = require('./paramTypes');
var responsesV3 = require("./helpers/v3/responses");
var paramsV3 = require("./helpers/v3/paramTypes");

function forOwn(target, iteratee) {
  if (!target) {
    return;
  }
...
function Swagger(appHandler) {
  ...
  this.responses = responsesV3;
  this.paramTypesV3 = paramsV3;
  this.runningInDev = false;
  this.appendToApi = appendToApi;
...
Swagger.prototype.isRunningInDev = function (callerRunningInDev) {
  var self = this;
  self.runningInDev = callerRunningInDev;
};
...
Swagger.prototype.configureSwaggerPaths = function (format, path, suffix) {
  ...
  //this.resourcePathV2 = "/api-docs-v2/";
...
Swagger.prototype.configure = function (bp, av, swaggerVersion) {
  var self = this;
  self.basePath = bp;
  self.apiVersion = av;
  self.swaggerVersion = swaggerVersion || "2.0";
  self.setResourceListingPaths(self.appHandler);

  console.log(`[btrz-swagger-express] Configured to use Swagger version ${self.swaggerVersion} schemas`);

  // hack for switching to use the v3 code
  if (self.swaggerVersion !== "2.0") {
    self.paramTypes = self.paramTypesV3;
    self.appendToApi = resourceHelpersV3.appendToApi;
  }
  // add the GET for resource listing
  var resourceListing = self.resourceListing.bind(self);
  self.appHandler.get(self.resourcePath.replace(self.formatString, self.jsonSuffix), resourceListing);

  // add the GET v2 for resource listing
  // var resourceListingV2 = self.resourceListingV2.bind(self);
  // self.appHandler.get(self.resourcePathV2.replace(self.formatString, self.jsonSuffix), resourceListingV2);

  // update resources if already configured

  forOwn(self.resources, function (resource) {
...
Swagger.prototype.setResourceListingPaths = function (app) {
  var self = this;
  forOwn(this.resources, function (resource, key) {
...
Swagger.prototype.addPropertiesToRequiredModels = function (properties, requiredModels) {
  ...
  forOwn(properties, function (property) {
...
Swagger.prototype.filterApiListing = function (req, res, r) {
  ...
  if (r.securityDefinitions) output.securityDefinitions = Object.assign({}, r.securityDefinitions);
  ...
  forOwn(apis, function (api) {
    var clonedApi = shallowClone(api);

    clonedApi.operations = [];
    forOwn(api.operations, function (operation) {
  ...
  output.models = {};
  forOwn(requiredModels, function (modelName) {
  ...
  forOwn(output.models, function (model) {
...
Swagger.prototype.addModelsFromBody = function (operation, models) {
  ...
  if (operation.parameters) {
    forOwn(operation.parameters, function (param) {
...
Swagger.prototype.canAccessResource = function (req, path, method) {
  for (var i = 0; i < this.validators.length; i++) {
    ...
    if (isFunction(validator) && !validator(req, path, method)) {
      return false;
    }
  }
  return true;
...
Swagger.prototype.resourceListing = function (req, res) {
  ...
  if (self.swaggerVersion === "2.0") {
    r["swagger"] = self.swaggerVersion;
    r["basePath"] = self.basePath;
    r["definitions"] = {};

    if (self.securityDefinitions) {
      r.securityDefinitions = self.securityDefinitions;
    }
    if (self.getModels) {
      r.definitions = self.getModels;
    }

    r.definitions = self.allModels;
  } else {
    r["openapi"] = self.swaggerVersion;
    r["servers"] = [{url: self.basePath}];
    r["components"] = {schemas: {}};
    if (self.getModels) {
      r.components.schemas = self.getModels;
    }

    r.components.schemas = self.allModels;
  }
  ...
  forOwn(self.resources, (value, key) => {
    value.apis.forEach((api) => {
      if (api.operations) {
        api.operations.forEach((operationValue) => {
          if (!operationValue.hideInDocumentation || self.runningInDev) {
            // if the tag has notbeen added, add it.
            if (!r.tags.some((k) => {
              return (k.name === key);
            })) {
              r.tags.push({
            ...
            if (self.swaggerVersion === "2.0") {
              openApi2ConversionHelpers.convertOperation(r, operationValue, key);
            } else {
              openApi3ConversionHelpers.convertOperation(r, operationValue, key);
            }
          }
        });
      }
...
Swagger.prototype.addHandlers = function (type, handlers) {
  var self = this;
  forOwn(handlers, function (handler) {
...
Swagger.prototype.discover = function (resource) {
  var self = this;
  forOwn(resource, function (handler, key) {
...
Swagger.prototype.addModels = function (models) {
  models = deepClone(models).models;
  var self = this;


  if (self.swaggerVersion === "2.0") {
    openApi2ConversionHelpers.convertModelsToOpenApi2(models);
  } else {
    openApi3ConversionHelpers.convertModels(models);
  }

  if (!self.allModels) {
    self.allModels = models;
  } else {
    forOwn(models, function (model, key) {
...
Swagger.prototype.configureDeclaration = function (resourceName, obj) {
  if (this.resources[resourceName]) {
    ...
    if (obj.securityDefinitions) {
      resource.securityDefinitions = obj.securityDefinitions;
    }
  }
};
