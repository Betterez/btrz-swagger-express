...
function getOperationMethod(operationPathValue) {
  ...
  // most likely a single key, but need too account for $ref and global parameters
...
function convertOperationToOpenApi2(doc, operationValue, key) {
  ...
  // if method has no value, assume native OpenApi 2 syntax
  if (!operationValue.path) {
    let method = getOperationMethod(operationValue[pathName]);
    doc.paths[pathName][method] = operationValue[pathName][method];
  } else {
    let method = operationValue.method.toLowerCase();
