    ...
    value.apis.forEach(function (api) {
      api.operations.forEach(function (operationValue) {
        if (!operationValue.hideInDocumentation || self.runningInDev) {
          ...
          if (!r.tags.some(function (k) { return (k.name === key); })) {
            r.tags.push({
              "name": key,
              "description": ""
            });
...
Swagger.prototype.addMethod = function (app, callback, spec) {
  var self = this;
  var specPath = openApi2ConversionHelpers.getSpecPath(spec);
  var apiRootPath = specPath.split(/[\/\(]/)[1];
  var root = self.resources[apiRootPath];

  if (root && root.apis) {
    var specMethod = openApi2ConversionHelpers.getSpecMethod(spec)
    _.forOwn(root.apis, function (api) {
      if (api && api.path === specPath && api.method === specMethod) {
  ...
  var fullPath = specPath.replace(self.formatString, self.jsonSuffix).replace(/\/{/g, '/:').replace(/\}/g, '');
  var currentMethod = specMethod.toLowerCase();
