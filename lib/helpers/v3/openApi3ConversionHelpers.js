...
function setResponses(docPathMethod, operationValue) {
  ...
  if (!isEmptyObject(operationValue.responses)) {
...
function convertModelsToOpenApi3(models) {
  ...
  forOwn(models, function (modelValue, key) {
    ...
    if (modelValue) {
      ...
      forOwn(modelValue.properties, function (propValue, propKey) {
...
function getQueryParam(paramValues) {
  return {
    ...
    // The "enum" belongs inside the "schema" property but is left here for backwards compatibility with older versions
