...
Swagger.prototype.convertModelsToOpenApi2 = function (doc) {
  var self = this;

  _.forOwn(doc.definitions, function (modelValue, key) {
    //id is not allowed in api spec 2
    if (doc.definitions[key]) {
      if (doc.definitions[key].id) {
        delete doc.definitions[key].id;
      }

      //required can't be included if it is empty
      if (doc.definitions[key].required && doc.definitions[key].required.length === 0) {
        delete doc.definitions[key].required;
      }

      _.forOwn(doc.definitions[key].properties, function (propValue, propKey) {
        if (doc.definitions[key].properties[propKey].type === "dateTime") {
          doc.definitions[key].properties[propKey].type = "string";
          doc.definitions[key].properties[propKey]["format"] = "date-time";

          //$ref: AccountPreferences
        }
        if(doc.definitions[key].properties[propKey]["$ref"]) {
          doc.definitions[key].properties[propKey]["$ref"] = "#/definitions/" + doc.definitions[key].properties[propKey]["$ref"];
        }
        if(doc.definitions[key].properties[propKey]["items"] && doc.definitions[key].properties[propKey]["items"]["$ref"]) {
          doc.definitions[key].properties[propKey]["items"]["$ref"] = "#/definitions/" + doc.definitions[key].properties[propKey]["items"]["$ref"];
        }
      });
...
Swagger.prototype.convertModelsToOpenApi2_2 = function (models) {
  var self = this;


  _.forOwn(models, function (modelValue, key) {

    // console.log("********************beforemodels*******************************");
    // console.log("key: ", key)
    // console.dir(modelValue, { depth: null });
    // console.log("********************beforemodels*******************************");
...
//transform the existing definition into a Swagger 2 friendly format
Swagger.prototype.convertOperationToOpenApi2 = function (doc, operationValue, key) {
  var self = this;
  var p = operationValue.path;
  var method = operationValue.method.toLowerCase();
  ...
  operationValue.errorResponses.forEach(function (response) {
    doc.paths[p][method]["responses"][response.code] = {
      "description": response.reason
    };
  });

  // console.log("**************", operationValue.parameters);
  // console.dir(operationValue, {depth:null});
  if (operationValue.parameters) {
    operationValue.parameters.forEach(function (param) {
      doc.paths[p][method]["parameters"].push({
        "name": param.name,
        "in": param.paramType,
...
Swagger.prototype.resourceListing = function (req, res) {
  ...
  // console.log("********************models*******************************");
  // console.log("typeof: ", typeof self.getModels)
  // //console.dir(self.getModels.HealthCheck, { depth: null });
  // console.log("********************models*******************************");
  var r = {
  ...
  if (self.getModels) { r.definitions = self.getModels }
  ...
  _.forOwn(self.resources, function (value, key) {
    ...
    value.apis.forEach(function (api) {
      api.operations.forEach(function (operationValue) {
        self.convertOperationToOpenApi2(r, operationValue, key);
      });
    });
...
// Swagger.prototype.resourceListingV2 = function(req, res) {
//   var self = this;
//   var r = {
...
Swagger.prototype.addMethod = function (app, callback, spec) {
  ...
  // console.log("***************************************************");
  if (root && root.apis) {
    // this path already exists in swagger resource
...
Swagger.prototype.addModels = function (models) {
  ...
  self.convertModelsToOpenApi2_2(models);
